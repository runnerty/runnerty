  ...
  return new Promise(async resolve => {
    if (dependencies instanceof Array) {
      ...
      for (let i = 0; i < dependencies.length; i++) {
        promArr.push(await _replaceStatusProcess(dependencies[i], chain_process));
      }
...
async function checkDependsProcess(depends_process_obj, chain_process, values) {
  if (depends_process_obj instanceof Object) {
    ...
    if (depends_process_obj[item] instanceof Array) {
      return await _checkOperator(depends_process_obj, chain_process, values);
    } else {
      return await _checkCondition(depends_process_obj, values);
    }
  } else {
...
function getAction(process, chain_process, values) {
  ...
  return new Promise((resolve, reject) => {
    ...
    _replaceStatusProcess(depends_process_obj, chain_process)
