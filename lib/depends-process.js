...
async function _match(key, value, values) {
  let res = false;
  key = await replaceWithSmart(key, values);
  value = await replaceWithSmart(value, values);
...
async function _checkCondition(objToCheck, values) {
  ...
  if (objToCheck instanceof Object) {
    ...
    if (keyVal instanceof Object) {
      ...
      key = await replaceWithSmart(key, values);
      keyVal = await replaceWithSmart(keyVal, values);
      // String
      if (key === '$end' || key === '$fail') {
  ...
  if (objOperator instanceof Object) {
    ...
    if (operatorConditions instanceof Array) {
      for (let i = 0; i < operatorConditions.length; i++) {
        let res = await checkDependsProcess(
  ...
  return new Promise(async resolve => {
    if (dependencies instanceof Array) {
      ...
      for (let i = 0; i < dependencies.length; i++) {
        promArr.push(
          await _replaceStausProcess(dependencies[i], chain_process)
        );
      }
      ...
      if (dependencies instanceof Object) {
        ...
        for (const key of keys) {
          switch (key) {
            ...
            case '$fail':
              resObject[key] =
                _returnProcessStatus(dependencies[key], chain_process) ||
                dependencies[key];
              break;
            default:
              resObject[key] = await _replaceStausProcess(
                dependencies[key],
                chain_process
              );
              break;
 ...
 * @returns {string}
 */
function checkDependsProcess(depends_process_obj, chain_process, values) {
  return new Promise(async (resolve, reject) => {
    if (depends_process_obj instanceof Object) {
      ...
      if (depends_process_obj[item] instanceof Array) {
        resolve(
          await _checkOperator(depends_process_obj, chain_process, values)
        );
      } else {
        resolve(await _checkCondition(depends_process_obj, values));
      }
    } else {
 ...
 * @returns {Promise}
 */
function getAction(process, chain_process, values) {
