    ...
    if (keyVal instanceof Object){
      ...
      if (key === "$end" || key === "$fail") {
        ...
        if(key === "$true" || key === "$false"){
          if((key === "$true" && (keyVal === "true" || keyVal === "1")) ||
  ...
  if (objOperator instanceof Object){
    ...
    if (operatorConditions instanceof Array){
      for (let i = 0; i < operatorConditions.length; i++){
        let res = await checkDependsProcess(operatorConditions[i], chain_process, values);
        switch (operator){
          case "$or":
  ...
  return new Promise(async (resolve, reject) => {
    if (depends_process_obj instanceof Object) {
      ...
      if (depends_process_obj[item] instanceof Array) {
        resolve(await _checkOperator(depends_process_obj, chain_process, values));
      } else {
 ...
 * @returns {Promise}
 */
function getAction(depends_process_obj, chain_process, values){
