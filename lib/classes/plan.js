...
class Plan {
        ...
        if (chainLength > 0) {
          ...
          while (chainLength--) {
            ...
            if (chain.disable) {
              logger.log(
                'warn',
                `Chain ${chain.id} ignored: is setted to disable.`
              );
            } else {
        ...
        sequence = sequence.then(() => {
          ...
          return _this
            ...
            .catch(err => {
              logger.log(
                'error',
                `scheduleChain createChainSerie loadChain ${chain.id}. Error: `,
                err
              );
            });
        });
        ...
        sequence = sequence.then(() => {
          if (!abortIteration) {
            ...
            return _chain
              ...
              .then(() => {
                if (
                  _chain.causedByAnProcessError &&
                  chain.abort_iteration_serie_on_error
                ) {
                  abortIteration = true;
                }
                i = i + 1;
      ...
      if (!executeInmediate && _this.hasDependenciesBlocking(chain)) {
        ...
        if (
          ...
          if (
            input_values &&
            input_values.length &&
            input_values instanceof String
          ) {
            try {
              inputIterable = JSON.parse(input_values);
            } catch (err) {
              reject(
                `Invalid input (${input_values}), incorrect JSON` +
                  '\nCaused by: ' +
                  err.stack
              );
            }
          ...
          if (inputIterable.length) {
            ...
            if (execMode === 'parallel') {
              ...
              createChainSerie(inputIterable).then(() => {
                ...
                while (chainsToExecLength--) {
                  ...
                  process.childs_chains.push(
                    process.childs_chains[chainsToExecLength].start(options)
                  );
                }
              ...
              if (
                ...
                logger.log(
                  ...
                  `Chain ${
                    chain.id
                  } abort_iteration_serie_on_error is setted to true because parent process ${
                    process.id
                  } fail_on_child_fail is true.`
                );
            ...
            logger.log(
              'debug',
              `input not found for iterable process ${process.id}`
            );
            resolve();
          }
        } else {
          ...
          if (
            chain.custom_values_overwrite &&
            Object.keys(chain.custom_values_overwrite).length !== 0
          ) {
            _this
              .loadChain(chain)
              .then(_chain => {
              ...
              .catch(err => {
                reject(
                  `scheduleChain customsValues loadChain ${
                    chain.id
                  }. Error: ${err}`
                );
              });
  ...
  dependenciesBlocking(chain) {
    ...
    if (
      chain.hasOwnProperty('depends_chains') &&
      chain.depends_chains.length > 0
    ) {
        ...
        if (depends_chains[dependsChainsLength]) {
          if (depends_chains[dependsChainsLength].hasOwnProperty('file_name')) {
            if (chain.depends_files_ready) {
              if (
                chain.depends_files_ready.indexOf(
                  depends_chains[dependsChainsLength].file_name
                ) < 0
              ) {
      ...
      while (planChainsLength--) {
        ...
        while (auxDependsChainsLength--) {
          switch (typeof depends_chains[auxDependsChainsLength]) {
            case 'string':
              if (
                depends_chains[auxDependsChainsLength] ===
                planChains[planChainsLength].id
              ) {
            ...
            case 'object':
              if (
                depends_chains[auxDependsChainsLength].chain_id ===
                planChains[planChainsLength].id
              ) {
                if (
                  ...
                  !(
                    ...
                    !depends_chains[auxDependsChainsLength].hasOwnProperty(
                      'process_id'
                    )
                  )
                ) {
          ...
          if (typeof depends_chains[auxDependsChainsLength] === 'object') {
            if (
              ...
              if (
                ...
                while (planProcessLength--) {
                  if (
                    planChains[planChainsLength].processes[planProcessLength]
                      .id === depends_chains[auxDependsChainsLength].process_id
                  ) {
                    const dep_process =
                      planChains[planChainsLength].processes[planProcessLength];
  ...
  getChainById(chainId, uniqueId, _pick) {
    ...
    function byId(chain) {
      return (
        chain.id === chainId &&
        (chain.uId === uniqueId || chain.execId === uniqueId)
      );
    }
