...
class Plan {
  ...
  async scheduleChainDependent(executeInmediate, chain, process, inputIterable, custom_values_overwrite) {
    try {
      let _chain = chain;
      chain.custom_values_overwrite = custom_values_overwrite;
      ...
      const options = { inputIteration: inputIterable[0], executeInmediate };
  ...
  async scheduleChain(chain, process, executeInmediate, input_values, custom_values_overwrite) {
    ...
    try {
      ...
      if (chain.isIterable()) {
        ...
        await this.scheduleChainDependent(executeInmediate, chain, process, inputIterable, custom_values_overwrite);
      }
    } catch (err) {
      throw err;
    }
  ...
  async startChain(chainId, uId, input_values, custom_values_overwrite, noWait) {
    ...
    if (chain) {
      if (chain.iterable) {
        ...
        if (noWait) {
          this.scheduleChain(chain, null, true, input_values, custom_values_overwrite);
        } else {
          await this.scheduleChain(chain, null, true, input_values, custom_values_overwrite);
        }
      }
    } else {
