...
class Execution {
  ...
  async end(options) {
    ...
    if (options.extra_output) {
      this.process.extra_output = utils.JSON2KV(
    ...
    switch (options.end) {
      case 'error':
        if (this.process.retries && !this.process.retries_count)
          this.process.retries_count = 0;

        // RETRIES:
        if (
          this.process.retries &&
          this.process.retries_count < this.process.retries
        ) {
          ...
          await this.process.error(
            !this.process.notificate_only_last_fail,
            writeOutput,
            willRetry
          );
          ...
          // RETRIES DELAY:
          setTimeout(() => {
            this.process.retries_count =
              (this.process.retries_count || 0) + 1;
