...
class Chain {
  constructor(chain, isMainChain) {
    ...
    this.ignore_on_concurrence =
      chain.ignore_on_concurrence || this.iterable ? false : true;
  ...
  historicize(event) {
    ...
    if (global.config.historyEnabled) {
      let mChain = new mongoChain(values);

      mChain.save(err => {
        if (err) {
          logger.log(
            'error',
            `Error historicize ${event} chain ${_this.id}`,
            err
          );
        }
      });
      ...
      if (process.isStopped()) {
        ...
        switch (processMustDo) {
                ...
                switch (action) {
                  ...
                  default:
                    if (process.chain_action_on_fail)
                      logger.log(
                        'warn',
                        `Ignored incorrect chain_action_on_fail declaration of process ${
                          process.id
                        }:`,
                        process.chain_action_on_fail
                      );
          ...
          case 'end':
            ...
            process.end(notificateEnd).then(() => {
              _this.startProcessesCtrl(options, resolve);
            });
            break;
          default:
