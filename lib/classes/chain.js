...
class Chain {
  constructor(chain, isMainChain) {
    ...
    this.ignore_on_concurrence = chain.ignore_on_concurrence === false ? false : this.iterable ? false : true;
      ...
      if (processes instanceof Array) {
        if (processesLength > 0) {
          while (processesLength--) {
            ...
            processes[processesLength].parentExecutionId = this.parentExecutionId || this.executionId;
  ...
  async refreshParentProcessChildsChainsStatus() {
    ...
    let processParentFound = getProcessByUId(globalPlanChains, this.parentProcessUId);
  ...
  async start(options) {
    ...
    if (options.inputIteration) {
      ...
      this.execute_input = await replaceWithSmart(this.input[0], this.values());
    }
