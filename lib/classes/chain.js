...
class Chain {
  constructor(chain, isMainChain) {
    const globalDefaultsChain = runtime.config?.defaults?.chain;
    const globalDefaultsProcess = runtime.config?.defaults?.process;
    this.id = chain.id;
    ...
    // Global defaults process:
    if (globalDefaultsProcess) {
      if (!this.defaults_processes) this.defaults_processes = {};
      // Notifications
      if (globalDefaultsProcess.notifications && !this.defaults_processes?.notifications) {
        this.defaults_processes.notifications = globalDefaultsProcess.notifications;
      } else {
        for (const notifEvent in globalDefaultsProcess.notifications) {
          if (!this.defaults_processes.notifications[notifEvent]) {
            this.defaults_processes.notifications[notifEvent] = globalDefaultsProcess.notifications[notifEvent];
          }
        }
      }
      // output:
      if (globalDefaultsProcess.output && !this.defaults_processes.output) {
        this.defaults_processes.output = globalDefaultsProcess.output;
      }

      //  chain_action_on_fail:
      if (globalDefaultsProcess.chain_action_on_fail && !this.defaults_processes.chain_action_on_fail) {
        this.defaults_processes.chain_action_on_fail = globalDefaultsProcess.chain_action_on_fail;
      }

      // ignore_in_final_chain_status:
      if (globalDefaultsProcess.ignore_in_final_chain_status && !this.defaults_processes.ignore_in_final_chain_status) {
        this.defaults_processes.ignore_in_final_chain_status = globalDefaultsProcess.ignore_in_final_chain_status;
      }
    }
    ...
    if (globalDefaultsChain) {
      if (globalDefaultsChain.notifications && !this.notifications) {
        this.notifications_plane = lodash.cloneDeep(globalDefaultsChain.notifications);
        this.notifications = lodash.cloneDeep(globalDefaultsChain.notifications);
      } else {
        for (const notifEvent in globalDefaultsChain.notifications) {
          if (!this.notifications[notifEvent]) {
            this.notifications[notifEvent] = globalDefaultsChain.notifications[notifEvent];
          }
        }
      }
    }
  ...
  async init() {
    try {
      // custom_values:
      this.custom_values_resolved = await recursiveObjectInterpreter(this.custom_values, this.values());
  ...
  values() {
    ...
    Object.assign(values, this.custom_values_resolved);
    return values;
  }
    ...
    for (const itemChain of runtime.plan.chains) {
      if (this.isChainDependient(itemChain)) {
        if (runtime.forcedInitChainsIds) {
          if (runtime.force_chain_dependents) {
    ...
    if (dependencies instanceof Array) {
      ...
      if (dependencies instanceof Object) {
        ...
        for (const key of keys) {
          if (dependsProcess.statusConditions.includes(key)) {
  ...
  async start(options) {
    ...
    // Refresh custom_values_resolved:
    this.custom_values_resolved = await recursiveObjectInterpreter(this.custom_values, this.values());
  ...
  async startProcess(process, options) {
    try {
      ...
      logger.log('error', 'startProcess error', err);
    }
  }
    ...
    try {
      ...
      if (chainStatus === 'running') {
        ...
        if (options.initialProcess) {
          ...
          // Get all dependents of initialProcess (when force_process_dependents):
          const initialDepens = runtime.force_process_dependents
            ? this.getDependentsOf(options.initialProcess, totalDeps)
            : [];
