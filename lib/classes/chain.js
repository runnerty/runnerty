...
class Chain {
  constructor(chain, isMainChain) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (isMainChain) {
        ...
        _this.triggers = await _this
          .loadChainTriggers(chain.triggers, _this)
          .catch(err => {
            reject(err);
          });
      ...
      _this.processes = await _this
        .loadProcesses(chain.processes, chain.defaults_processes)
        .catch(err => {
          reject();
        });
      _this.notifications = await _this
        .loadChainNotifications(chain.notifications)
        .catch(err => {
          reject();
        });
        ...
        if (triggersLength > 0) {
          while (triggersLength--) {
            ...
            if (trigger.id) {
              ...
              reject(
                `Trigger without id: check the trigger configuration of the chain ${chain.id}`
              );
            }
          }
  ...
  values() {
    ...
    let chain_values = {
      ...
      CHAIN_RETRIES_COUNT: _this.retries_count || 0,
      CHAIN_RETRIES: _this.retries
    };
    let values = {};
  ...
  historicize(event) {
    ...
    return new Promise(async resolve => {
      ...
      if (global.runnertyio.debug) {
        logger.log(
          'info',
          `DEBUG-runnerty.io [pre-send] from CHAIN ${_this.id} (${_this.uId}).`
        );
      }
  ...
  error() {
    ...
    return new Promise(async resolve => {
      if (!_this.isErrored()) {
        ...
        await _this.notificate('on_fail');
        await _this.historicize();
        _this.causedByAnProcessError = true;
        _this.refreshParentProcessChildsChainsStatus();
        _this.killRunnertyOnForcedInitChainsStop();
      }
      resolve();
    });
  }
  ...
  async running() {
    ...
    await _this.historicize('start');
  }

  async queue_up() {
    let _this = this;
    await _this.notificate('on_queue');
    await _this.historicize('queue');
  }
  ...
  retry(options) {
    ...
    setTimeout(async () => {
      await _this.notificate('on_retry');
      _this
        .setChainToInitState()
        .then(() => {
          _this
  ...
  async waiting_dependencies() {
    let _this = this;
    await _this.notificate('on_waiting_dependencies');
  }
          ...
          case 'run':
            ...
            if (process.isRunning()) {
              ...
              process
                .start()
                .then(() => {
                  // console.log("e1 FIN OK de ",process.id);
                  process
                ...
                .catch(err => {
                  ...
                  let functEnd = async () => {
                    ...
                    await _this.end(causedByAnProcessError);
                    resolve();
                  };
                 ...
                 // console.log("r1.- retries: ",action, options.retries);
                  switch (action) {
                    case 'abort':
                      functEnd();
                      break;
                    case 'retry':
                      ...
                      // console.log("r2.- retries: ",action,options.retries);
  ...
  checkProcessActionToDo(process) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (process.hasOwnProperty('depends_process')) {
        dependsProcess
          .getAction(process, _this.processes, _this.values())
          .then(res => {
            if (
              res === 'run' &&
              process.queue &&
    ...
    if (global.forcedInitChainsIds && global.endOnforcedInitChainsIds) {
      for (let i = 0; i < global.forcedInitChainsIds.length; i++) {
        if (global.forcedInitChainsIds[i] === this.id) {
            setTimeout(() => {
              if((this.retries && this.status === 'error' && this.retries_count === this.retries) || ((!this.retries && this.status === 'error') || this.status === 'end'))
              {
