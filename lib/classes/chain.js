...
class Chain {
  constructor(chain, isMainChain) {
    ...
    return new Promise(async resolve => {
      ...
      if (isMainChain){
        _this.uId = "main";
        _this.triggers = await _this.loadChainTriggers(chain.triggers, chain);
        if(_this.triggers) {
    ...
    return new Promise(async (resolve) => {
      if (global.config.historyEnabled) {
        ...
        ({
          ...
          execId: await replaceWithSmart(_this.execId),
          name: _this.name,
          queue: _this.queue,
      ...
      if (_this.hasOwnProperty("processes")) {
        if (_this.processes instanceof Array && _this.processes.length > 0) {
          if (options.executeInmediate || options.inputIteration) {
            ...
            //TODO: revisar para que se estaba usando esto:
            /*
            if(_this.triggers){
              _this.triggers.forEach(trigger => {
                trigger.start();
