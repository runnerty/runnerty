...
class FilePlan {
  ...
  async init(namespace) {
    try {
      if (this.planContent) {
        ...
        const chains = await this.loadChains(namespace);
        return chains;
      } else {
  ...
  async loadChains(namespace) {
    try {
      const chains = await this.getChains(this.fileContent, namespace);
      if (namespace && !chains.length) {
        throw new Error(`No chain found for the indicated namespace: ${namespace}`);
      }
