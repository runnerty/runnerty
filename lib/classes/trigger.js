...
class Trigger {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (_this.params.server) {
        if (global.servers[_this.params.server.id]) {
          _this.params.server = Object.assign(
            _this.params.server,
            global.servers[_this.params.server.id]
          );
        } else {
          reject(
            `Trigger Error. Server Id ${_this.params.server.id} not found.`
          );
        }
      }
  ...
  start() {
    ...
    return new Promise((resolve, reject) => {
      if (_this.params.server) {
        _this.params.server.router[_this.params.server.method.toLowerCase()](
          _this.params.server.path || '/',
          (req, res) => {
        ...
        logger.log(
          'error',
          'Method start (execution) must be rewrite in child class'
        );
        _this.process.execute_err_return =
          'Method start (trigger) must be rewrite in child class';
  ...
  on_request() {
    ...
    return new Promise((resolve, reject) => {
      logger.log(
        'error',
        'Method on_request (execution) must be rewrite in child class'
      );
      _this.process.execute_err_return =
        'Method on_request (trigger) must be rewrite in child class';
      _this.process.msg_output = '';
      _this.process.error();
  ...
  startChain(
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (customValues) {
        Object.assign(
          _this.chain.custom_values || {},
          await utils.objToKeyValue(customValues)
        );
      }
        ...
        checkCalendar(_this.params.calendars)
          .then(dateEnableOnDate => {
            if (dateEnableOnDate) {
              ...
              logger.log(
                'debug',
                `Chain ${_this.id} not started: Date not enable in calendar.`
              );
              resolve();
            }
          })
          .catch(err => {
            ...
            logger.log(
              'debug',
              `Chain ${_this.id} not started - checking calendars: ${err}`
            );
            reject(err);
          });
      } else {
