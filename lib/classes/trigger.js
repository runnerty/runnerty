...
class Trigger {
    ...
    try {
      ...
      if (this.params.server) {
        if (global.servers[this.params.server.id]) {
          this.params.server = Object.assign(
            this.params.server,
            global.servers[this.params.server.id]
          );
        } else {
  ...
  async start() {
    if (this.params.server) {
      this.params.server.router[this.params.server.method.toLowerCase()](
        this.params.server.path || '/',
        (req, res) => {
      ...
      this.process.execute_err_return =
