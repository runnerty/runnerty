...
class Process {
  constructor(process) {
    ...
    this.ignore_in_chain_status = process.ignore_in_chain_status || false;
  ...
  values() {
    ...
    let process_values = {
      ...
      PROCESS_PARENT_EXECUTION_ID: _this.parentExecutionId,
      ...
      PROCESS_EXEC_OUTPUT_SHARE: _this.execute_output_share,
      ...
      PROCESS_FIRST_DEPEND_FILE_READY: _this.depends_files_ready && _this.depends_files_ready.length > 0 ?
        _this.depends_files_ready[0] : [],
      PROCESS_LAST_DEPEND_FILE_READY: _this.depends_files_ready && _this.depends_files_ready.length > 0 ?
  ...
  historicize(event) {
    ...
    return new Promise(resolve => {
      ...
      const values = {
        ...
        parentExecutionId: _this.parentExecutionId,
        ...
        output_share: _this.execute_output_share,
        ...
        output_size: sizeof(_this.msg_output || '') +
      ...
      global.runnertyio.send('process', valuesSec).catch(err => {
        logger.log(
          'error',
          `RunnertyIO - error historicize ${valuesSec.event} process ${_this.id} ${err}.\n values:${valuesSec}`
        );
      });
      resolve();
    });
    ...
    return new Promise(resolve => {
      if (_this.hasOwnProperty('output_share') && _this.output_share) {
        ...
        replaceWithSmart(_this.output_share, _this.values(), options).then(
          res => {
            _this.execute_output_share = res;
            res.forEach(valOS => {
              let _valOS = {};
              _valOS[valOS.key] = {};
          ...
          if (err) {
            ...
            fs.open(filePath, mode, (err, fd) => {
              if (err) {
                logger.log(
                  'error',
                  `Writing output, open file ${filePath} in ${_this.id}: ${err}`
                );
                reject(err);
              } else {
                fs.write(fd, os, null, 'utf8')
                  .then(() => {
                    fs.close(fd, err => {
                      if (err) {
                        logger.log(
                          'error',
                          `Closing file ${filePath} in writeFile in ${_this.id}: ${err}`
                        );
                        reject(err);
                      } else {
                        resolve();
                  ...
                  .catch(err => {
                    logger.log(
                      'error',
                      `Writing output file ${filePath} in ${_this.id}: ${err}`
                    );
                    reject(err);
                  });
              }
                          ...
                          (error, bytesRead, buffer) => {
                            ...
                            fs.close(fd, err => {
                              if (err) {
                                logger.log(
                                  'error',
                                  `Closing file ${filePath} in ${_this.id}: ${err}`
                                );
                              }
  ...
  clean() {
    ...
    delete _this.execute_output_share;
