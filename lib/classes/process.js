      ...
      if (_this.hasOwnProperty("notifications") && _this.notifications !== undefined) {
        if (_this.notifications.hasOwnProperty(event)) {
          ...
          _this.notifications[event].map(_notification => {
            notificationsPromises.push(_notification.notificate(_this.values()));
          });
        ...
        replaceWithSmart(_this.output_share, _this.values(), options)
          .then((res) => {
            res.forEach((valOS) => {
              ...
              if(!global.config_raw.global_values){
                global.config_raw.global_values = [];
              }

              let gvrresLength = global.config_raw.global_values.length;
              let vrReplaced = false;

              for (let i = 0; i < gvrresLength; i++) {
                let valKey = Object.keys(_valOS)[0];
                let gvKey = Object.keys(global.config_raw.global_values[i])[0];
                if(valKey === gvKey){
                  global.config_raw.global_values[i] = lodash.defaultsDeep(_valOS, global.config_raw.global_values[i]);
                  vrReplaced = true;
                }
              }

              if(!vrReplaced){
                global.config_raw.global_values.push(_valOS);
              }
                  ...
                  fs.stat(filePath, (error, stats) => {
                    ...
                    if (totalSizeToWrite > maxSizeBytes) {
                      ...
                      fs.open(filePath, "r", (error, fd) => {
                        if (lengthFileRead > 0) {
                          let buffer = new Buffer(lengthFileRead);
