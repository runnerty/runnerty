...
class Process {
  constructor(process) {
    ...
    this.depends_process_alt = process.depends_process_alt;
    ...
    this.depends_conditions = process.depends_conditions;

    //Runtime attributes:
    this.status = process.status || "stop";
    this.execute_return = process.execute_return;
    this.execute_err_return = process.execute_err_return;
    ...
    return new Promise((resolve, reject) => {
      ...
      _this.setUid()
        .then(() => {
          _this.loadEvents(process.events)
            .then((events) => {
              _this.events = events;
              resolve(this);
            })
        ...
        replaceWithSmart(_this.output_share, _this.values(), options)
          .then((res) => {
            res.forEach(function (valOS) {
              ...
              global.config.global_values = lodash.defaultsDeep([_valOS], global.config.global_values);
            });
            resolve();
          });
      }else{
