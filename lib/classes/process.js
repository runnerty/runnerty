...
class Process {
  ...
  startChildChainsDependients() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const output_iterable_string = _this.values()[_this.output_iterable];
      ...
      if (childChainsinputIterable && childChainsinputIterable.length > 0) {
        while (chainsLength--) {
          const itemChain = global.runtimePlan.plan.chains[chainsLength];
          const procValues = _this.values();
          ...
          if (
            ...
            const childChain = global.runtimePlan.plan.scheduleChain(
              itemChain,
              _this,
              executeInmediate,
  ...
  async setOutputShare() {
    if (this.hasOwnProperty('output_share') && this.output_share) {
      ...
      this.execute_output_share.forEach(valOS => {
        ...
        const gvresLength = global.config.global_values.length;
        let vReplaced = false;

        for (let i = 0; i < gvresLength; i++) {
          const valKey = Object.keys(_valOS)[0];
          const gvKey = Object.keys(global.config.global_values[i])[0];
          if (valKey === gvKey) {
  ...
  start(isRetry) {
    return new Promise(async (resolve, reject) => {
      ...
      if (this.exec.id) {
        this.loadExecutorConfig()
          .then(async configValues => {
            if (configValues.type) {
              if (global.executors[configValues.type]) {
                const executor = new global.executors[configValues.type](this);
                executor
                  .init()
                  .then(res => {
                    this.executor = res;
          ...
          .catch(async err => {
            this.err_output = `Process start loadExecutorConfig: ${err}`;
            this.msg_output = '';
            await this.error();
            reject(err);
          });
  ...
  async write_output() {
    ...
    async function writeFile(filePath, mode, os) {
    ...
    async function generateOutput(output) {
      try {
        const _output = await replaceWithSmart(output, _this.values());
                ...
                if (totalSizeToWrite > maxSizeBytes) {
                  ...
                  fs.open(filePath, 'r', async (err, fd) => {
                    ...
                    if (lengthFileRead > 0) {
                      ...
                      fs.read(fd, buffer, 0, buffer.length, positionFileRead, (err, bytesRead, buffer) => {
                        if (err) throw err;
                        let data = buffer.toString('utf8', 0, buffer.length);
                        data = data.concat('\n', output_stream);
                        fs.close(async (fd, err) => {
                          if (err) {
                            logger.log('error', `Closing file ${filePath} in ${_this.id}: ${err}`);
                          }
                          await writeFile(filePath, 'w', data);
                        });
                      });
                    } else {
                      //IF WILL NOT WRITE ANYTHING OF THE CURRENT FILE:
                      await writeFile(filePath, 'w', output_stream);
                    }
                  });
                } else {
                  await writeFile(filePath, 'a+', output_stream);
                }
              });
            } else {
              await writeFile(filePath, 'a+', output_stream);
    ...
    try {
      if (_this.output instanceof Array) {
        ...
        await generateOutput(_this.output);
      }
    } catch (err) {
      throw err;
    }
