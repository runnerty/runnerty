  ...
  values() {
    const process_values = {
      ...
      PROCESS_EXEC_COMMAND:
        this.exec instanceof Object ? this.exec.command : this.exec,
      ...
      if (this.notifications.hasOwnProperty(event)) {
        ...
        this.notifications[event].map(_notification => {
          notificationsPromises.push(
            _notification.notificate(this.values())
          );
        });
