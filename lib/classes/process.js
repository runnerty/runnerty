...
const runnertyio = require('./runnerty-io.js');
...
const notificationEvent = require('./notification-event.js');
...
class Process {
  constructor(process) {
    ...
    this.parentUId = process.chain.uId;
    this.parentExecutionId = process.chain.parentExecutionId || process.chain.executionId;
    this.executionId = process.chain.executionId || '';
    ...
    this.custom_values = process.chain.custom_values || {};
  ...
  values() {
    ...
    const process_values = {
      ...
      PROCESS_PARENT_UID: this.chain.uId,
      PROCESS_PARENT_EXECUTION_ID: this.chain.parentExecutionId || this.chain.executionId,
      PROCESS_EXECUTION_ID: this.chain.executionId,
    ...
    Object.assign(values, this.chain.custom_values);
    return values;
  }
  ...
  historicize(event) {
    const values = {
      id: this.id,
      uId: this.uId,
      parentUId: this.chain.uId,
      parentExecutionId: this.chain.parentExecutionId || this.chain.executionId,
      executionId: this.chain.executionId,
      ...
      custom_values: this.chain.custom_values,
    ...
    // RunnertyIO History
    if (this?.chain?.runnertyio?.sync?.events !== false) {
  ...
  async endChildChains(abortIfEndCausedByAnProcessError) {
    ...
    const chainParentFound = getChainByUId(globalPlanChains, this.chain.uId);
    if (chainParentFound) {
  ...
  isOwnDependsChains(depends_chains) {
    if (
      ...
      depends_chains.chain_id === this.values()['CHAIN_ID'] &&
      depends_chains.process_id === this.id
    ) {
      return true;
    }
    return false;
  ...
  isChainChild(chainToCheck) {
    ...
    if (chainToCheck.hasOwnProperty('depends_chains') && chainToCheck.depends_chains !== null) {
      ...
      if (chainToCheck.depends_chains instanceof Object) {
        isChild = this.isOwnDependsChains(chainToCheck.depends_chains);
      }
      // Array:
      if (chainToCheck.depends_chains instanceof Array) {
        for (const chainToCheckDeps of chainToCheck.depends_chains) {
          if (this.isOwnDependsChains(chainToCheckDeps)) isChild = true;
        }
      }
    }
    return isChild;
  ...
  async startChildChainsDependients() {
    ...
    // Prepare input for chain child not iterable from output_share:
    const execute_output_share_formated = {};
    if (this.output_share) {
      const options = {
        ignoreGlobalValues: false,
        altValueReplace: ''
      };
      const execute_output_share = await recursiveObjectInterpreter(this.output_share, this.values(), options);
      for (const output_share_item of execute_output_share) {
        execute_output_share_formated[output_share_item.key + '_' + output_share_item.name] = output_share_item.value;
      }
    }

    while (chainsLength--) {
      const itemChain = runtime.plan.chains[chainsLength];
      const itemChainIsIterable = itemChain.iterable !== undefined;

      const childMustStart = true;
      if (runtime.forcedInitChainsIds && !runtime.force_chain_dependents) {
        childMustStart = false;
      }

      if (
        this.isChainChild(itemChain) &&
        ((itemChainIsIterable && childChainsinputIterable?.length > 0) || // child iterable and output have items to iter
          !itemChainIsIterable) && // child is not iterable
        childMustStart
      ) {
        ...
        // If chain child is not iterable the chain input is the process output_share:
        if (!itemChainIsIterable && Object.keys(execute_output_share_formated).length >= 0) {
          childChainsinputIterable = [execute_output_share_formated];
        }
