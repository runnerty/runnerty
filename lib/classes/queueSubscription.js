...
class queueSubscription {
      ...
      if (next.chainId) {
        globalPlan
          ...
          .catch(err => {
            logger.log(
              'error',
              `queueSubscription startExecutionQueue: startChain ${
                next.chainId
              } / ${next.uId}`,
              err
            );
    ...
    if (next) {
      globalPlan
        ...
        .catch(err => {
          logger.log(
            'error',
            `queueSubscription startExecutionQueueZero: startChain ${
              next.chainId
            } / ${next.uId} - ${next.id}`,
            err
          );
  ...
  setRunningQueue(chainData) {
    ...
    if (!global.processQueues[_this.queue + '_RUN']) {
      global.processQueues[_this.queue + '_RUN'] = {};
    }

    global.processQueues[_this.queue + '_RUN'][chainData.chainId] = chainData;
  }
}
