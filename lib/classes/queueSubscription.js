...
class queueSubscription {
    ...
    if (next) {
      if (next.chainId) {
        try {
          ...
          await runtime.plan.startChain(next.chainId, next.uId, next.input_values, next.custom_values_overwrite);
        ...
        await chain.startProcesses(options, next.processId);
  ...
  getNext() {
    if (runtime.processQueues[this.queue]) {
      ...
      runtime.processQueues[this.queue] = [];
      return null;
    }
  }
