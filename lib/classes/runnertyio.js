...
class Runnertyio {
  ...
  static init(config) {
        ...
        if (this.syncPlan) {
          try {
            if (runtime.planePlanChains) {
              ...
              planTyped = this.setPlanTypes(plan);
            }
          } catch (err) {
            throw new Error(err);
          }
    ...
    this.socket.on('connect_error', error => {
      ...
      logger.log(
        'warn',
        `connect_error to runnerty.io. The following access attempts could be ignored.\n${error.message}`
      );
    });
  ...
  static runCommand(data) {
    ...
    if (data.command === 'kill/chain' && data.chainId) {
      runtime.plan.stopChain(data.chainId, data.uniqueId);
    }
  }
}
