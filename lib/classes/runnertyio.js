...
class Runnertyio {
  ...
  static init(config) {
    ...
    this.socket.on('connect', () => {
      this.socket.emit(this.events['access'], version);
    ...
    this.socket.on('disconnect', () => {
      this.haveAccess = false;
      logger.log('error', `Lost connection with runnerty.io: The following access attempts could be ignored.`);
    });
    ...
    this.socket.on('remote-commands', data => {
      console.log('remote-commands:', data);
