...
class Runnertyio {
  ...
  static async init(config) {
    ...
    this.syncPlan = config.syncPlan !== false;
    this.waitForConnection = config.waitForConnection !== false;
    this.connectionTimeout = config.connectionTimeout;
    this.haveAccess = false;
    this.firstConnection = true;
    this.connectionTimeoutExceeded = false;
    ...
    this.socket.on('remote-commands', data => {
      if (this.debug) logger.log('info', `DEBUG-runnerty.io [remote-commands].\nData:${JSON.stringify(data)}`);
  ...
  static healthCheck() {
    if (this.haveAccess) {
      ...
      logger.log('warn', `There is no access to runnerty.io. (Health Check).`);
    }
  }
  ...
  static runCommand(data) {
    ...
    if (data.command === 'run/chain' && data.chainId) {
      ...
      if (chain && chain?.runnertyio?.remoteControl !== false) {
        queueProcess.queueChain(chain, data.input || [], data.custom_values || {});
      } else if (!chain) {
        logger.log('warn', `Remote command (${data.command}) for chain not found:`, data.chainId);
      }
    }
  }

  static async checkConnectionTimeout() {
    if (this.connectionTimeout) {
      await sleep(this.connectionTimeout);
      if (!this.haveAccess) {
        this.connectionTimeoutExceeded = true;
        if (this.debug) logger.log('warn', `Connection timeout exceeded for connection to runnerty.io.`);
      }
    }
  }

  static async checkConnection() {
    if (this.waitForConnection && !this.haveAccess && !this.connectionTimeoutExceeded) {
      if (this.debug) logger.log('info', `Waiting for connection to runnerty.io... `);
      await sleep(250);
      await this.checkConnection();
    }
  }
