...
function getENVs(s) {
  let res = "";
  let envName = s.substr(4, s.length);
 ...
 * @returns {string}
 */
function getvalue(idvalue, values) {
  let match = values[idvalue];
  let res = "";
...
function random(round, min, max) {
  ...
  if(round || round === 0){
    res = parseFloat(((min || min === 0) && (max || max === 0))?(Math.random() * (max+1 - min) + min):Math.random()).toFixed(round);
  }else{
 ...
 * @returns {string}
 */
function pathParse(_path, propertie) {
  const pathProperties = path.parse(_path);
  let res = "";

  if(pathProperties[propertie]){
    res = pathProperties[propertie];
  }else{
    logger.log("warn", `PathParse (${_path}) worng propertie ${propertie}.`);
  }
  return res;
}
 ...
 * @returns {string}
 */
function urlParse(_url, propertie) {
  const urlProperties = url.parse(_url);
  let res = "";

  if(urlProperties[propertie]){
    res = urlProperties[propertie];
  }else{
    logger.log("warn", `UrlParser (${_url}) worng propertie ${propertie}.`);
  }
  return res;
}
