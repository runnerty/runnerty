...
function pathParse(_path, property) {
  ...
  if(pathProperties[property]){
    ...
    logger.log("warn", `PathParse (${_path}) wrong property ${property}.`);
  }
  return res;
}
...
function urlParse(_url, property) {
  ...
  if(urlProperties[property]){
    ...
    logger.log("warn", `UrlParser (${_url}) wrong property ${property}.`);
  }
  return res;
}
...
function stringify(obj) {
  if (!obj) {
    logger.log("debug", `Interpreter: stringifying non value variable -> ${obj}`);
    return "";
  } else {
    try {
      return JSON.stringify(obj);
    } catch (ex) {
      logger.log("error", `Interpreter: Stringify exception: ${ex}`);
      return "";
    }
  }
}
