...
function getvalue(idvalue, quote, values) {
  ...
  if(values[idvalue]){
    ...
    throw `value for value id "${idvalue}" not found.`;
  }
  return res;
}
...
function pathParse(_path, property) {
  ...
  if(pathProperties[property]){
    ...
    throw `PathParse (${_path}) wrong property ${property}.`;
  }
  return res;
}
...
function urlParse(_url, property) {
  ...
  if(urlProperties[property]){
    ...
    throw `UrlParser (${_url}) wrong property ${property}.`;
  }
  return res;
}
...
function stringify(obj) {
  if (!obj) {
    throw `stringifying non value variable -> ${obj}`;
  } else {
    try {
      return JSON.stringify(obj);
    } catch (ex) {
      throw `Stringify exception: ${ex}`;
    }
  }
}
