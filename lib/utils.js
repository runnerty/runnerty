          ...
          fs.readFile(filePath, "utf8", async (err, res) => {
            if (err) {
              ...
              validateConfig(configLoad)
                .then(() => {
                  global.config_raw = {};
                  global.config_raw.global_values = configLoad.config.global_values;
...
function addGlobalValuesToObjParams(objParams) {
  ...
  return new Promise(async (resolve) => {
    ...
    const gvs = await replaceWithSmart(global.config_raw.global_values,{},rw_options);
    let res = {};
  ...
  return new Promise(resolve => {
    ...
    switch (true) {
      case !!server.ssl && !!server.key && !!server.cert && server.port:
        ...
        srv = https.createServer({ key: privateKey, cert: certificate }, app);
        port = server.port;
        break;
