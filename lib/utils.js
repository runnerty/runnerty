...
function loadConfigModules(mergedConfig, configFilePath){
  return new Promise((resolve, reject) => {
    ...
    if(mergedConfig.general.modulesPath){
      modulesPath = path.join(mergedConfig.general.modulesPath, "node_modules");
    }else{
      if(configFilePath){
        modulesPath = path.join(path.dirname(configFilePath), "node_modules");
      }else{
    ...
    const notificatorsPath = mergedConfig.general.notificatorsPath || modulesPath;
    const promiseNotificatorsSchemas = loadNotificators(notificatorsPath, mergedConfig.notificators);

    // ADD EXECUTORS SCHEMAS:
    const executorsPath = mergedConfig.general.executorsPath || modulesPath;
    const promiseExecutorsSchemas = loadExecutors(executorsPath, mergedConfig.executors);

    // ADD TRIGGERS SCHEMAS:
    const triggersPath = mergedConfig.general.triggersPath || modulesPath;
    const promiseTriggersSchemas = loadTriggers(triggersPath, mergedConfig.triggers);
...
// If force execution chain on Start
  if(program.force_chain_exec){
    ...
    apiPlan.startChain(chainId, "main", inputValues, customValues)
      .then(() => {
