...
function replaceWithSmart(inputObject, objParams, options) {
  ...
  function execSerie(keys, resObject) {
      ...
      if (intialValue instanceof Object) {
        ...
        if (intialValue.format === "text") {
          ...
          if (intialValue.value instanceof Array) {
            ...
            gv[keymaster + "_" + keysValueObjects[keysValueObjectsLength]] = finalValue;

          } else {
            let value = replaceWith(intialValue.value, objParams, true);
            gv[keymaster + "_" + keysValueObjects[keysValueObjectsLength]] = value;
          }
          ...
          if (intialValue.format === "json") {
            ...
            if (intialValue.value instanceof Object || intialValue.value instanceof Array) {
              gv[keymaster + "_" + keysValueObjects[keysValueObjectsLength]] = replaceWith(JSON.stringify(intialValue.value), objParams, true);

            } else {
              gv[keymaster + "_" + keysValueObjects[keysValueObjectsLength]] = replaceWith(intialValue.value, objParams, true);
            }
          }
        }

      } else {
        gv[keymaster + "_" + keysValueObjects[keysValueObjectsLength]] = replaceWith(intialValue, objParams, true);
      }
    }
  }
    ...
    fs.readdir(containerDirectory, function (err, items) {
      if (err) {
        ...
        if(items){
          ...
          var dirs = dirsItems ? dirsItems.filter(function (i) {
  ...
  return new Promise((resolve) => {
    if (config.general.history && config.general.history.mongodb && (config.general.history.disable !== true)) {
      ...
      mongoose.connect(`mongodb://${config.general.history.mongodb.host}:${config.general.history.mongodb.port||"27017"}/${config.general.history.mongodb.database}`)
        .then(() => {
          logger.log("info", "Mongo History is enabled.");
          resolve();
        },
        err => {
          resolve();
