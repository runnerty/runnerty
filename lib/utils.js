...
async function forceInitChainExecution(program) {
  ...
  if (program.force_chain_exec) {
    ...
    if (program.forceDependents) runtime.forceDependents = true;
    ...
    for (const chainId of chainsIds) {
      ...
      if (_res) {
        ...
        const procFound = chain.processes.find(proc => proc.id === initialProcess);
        if (initialProcess && !procFound) {
          throw new Error(
            `The process you are trying to run (${initialProcess}) has not been found in the chain (${chain.id}). Please check the process ID.`
          );
        }

        queue.queueChain(chain, input_values, custom_values_override, initialProcess);
      } else {
        throw new Error(
