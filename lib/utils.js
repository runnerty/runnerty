    ...
    fs.stat(configFilePath, (err, stats) => {
      if (err || !stats.isFile()) {
        if (stats && !stats.isFile()) {
          throw new Error(`config.json must be file but is set ${configFilePath}`);
        } else {
          throw new Error(`Load General conf file ${configFilePath} not exists.`);
        }
      } else {
        try {
          fs.readFile(configFilePath, 'utf8', async (err, res) => {
            if (err) {
          ...
          throw new Error(`Invalid Config file: ${err} ${err.message}`);
        }
      }
    });
  });
...
function replaceWithSmart(inputObject, objParams = {}, options) {
  return new Promise(async resolve => {
    ...
    if (typeof inputObject === 'string') {
      let res = interpretAwait(objParams, inputObject, params, options);
      resolve(res);
    } else {
      if (inputObject instanceof Array) {
        ...
        if (inputObject instanceof Object) {
          ...
          for (const key of keys) {
            ...
            let _key = interpretAwait(objParams, key, params, options);
      ...
      if (chain.processes && chain.processes.length) {
        ...
        while (chainProcessesLength-- && !res) {
          ...
          if (process.childs_chains) {
            _res = getChainByUId(process.childs_chains, uId);
            if (_res) {
              res = _res;
            }
          }
...
async function loadTrigger(chain, triggerParams) {
  try {
    ...
    const trigger = new global.triggers[configValues.type](chain, triggerParams);
