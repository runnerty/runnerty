...
async function generateCalendar(calendarData, name) {
  if (fs.existsSync(calendarData)) {
    const fileName = name || path.parse(calendarData).name;
    const fileExt = path.parse(calendarData).ext;
    if (fileExt === '.ics') {
      const parsedCal = await ical.async.parseFile(calendarData);
      const calEvents = [];
    ...
    if (isUrl(calendarData)) {
      const parsedCal = await ical.async.fromURL(calendarData);
      const calEvents = [];
      for (const event in parsedCal) {
        calEvents.push(parsedCal[event]);
      }
      runtime.calendars[name] = calEvents;
    } else {
      logger.log('error', `Calendar file not found: ${calendarData}`);
    }
  }
}
