...
module.exports.loadGeneralConfig = async function loadGeneralConfig(configFilePath) {
  try {
    const stats = await fsp.stat(configFilePath);
    if (!stats.isFile()) {
    ...
    throw new Error(`Load General conf file ${err}.`);
  }
};
...
module.exports.loadSQLFile = async function loadSQLFile(filePath) {
  try {
    const sqlFileStat = await fsp.stat(filePath);
    if (!sqlFileStat.isFile()) throw new Error(`not found ${filePath}`);
...
module.exports.loadCalendars = async function loadCalendars() {
  ...
  if (global.config.general.calendarsPath) {
    try {
      const files = fsp.readdir(global.config.general.calendarsPath);
