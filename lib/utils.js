...
function getENVs(text) {
  return new Promise(resolve => {
    ...
    let envs = text.toString().match(/\:ENV_\w+/g);
    if (envs) {
...
function getObjsCustomDates(text){
  ...
  return new Promise(async (resolve) => {
    ...
    let yearsInc = text.toString().match(/\:YYYY_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (yearsInc) {
      let yearsIncLength = yearsInc.length;

      while (yearsIncLength--) {
        let year = yearsInc[yearsIncLength].substr(1, 7);
        let yearInc = yearsInc[yearsIncLength].substr(6, year.length);
        let incremetObj = {};
    ...
    // YEARS YY_[INCREMENT]
    let shortYearsInc = text.toString().match(/\:YY_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (shortYearsInc) {
      let shortYearsIncLength = shortYearsInc.length;

      while (shortYearsIncLength--) {
        let shortYear = shortYearsInc[shortYearsIncLength].substr(1, 5);
        let shortYearInc = shortYearsInc[shortYearsIncLength].substr(4, shortYear.length);
        let incremetObj = {};
        incremetObj.period = "years";
    ...
    // MONTHS MMMM_[LANG]
    let months = text.toString().match(/\:MMMM_[a-zA-Z]{2}/ig);

    if (months) {
      let monthsLength = months.length;

      while (monthsLength--) {
        let month = months[monthsLength].substr(1, 7);
        let monthLang = months[monthsLength].substr(6, 2);
        if (!objParams[month]) {
    ...
    let monthsInc = text.toString().match(/\:MMMM_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (monthsInc) {
      var monthsIncLength = monthsInc.length;

      while (monthsIncLength--) {
        let month = monthsInc[monthsIncLength].substr(1, 7);
        let monthInc = monthsInc[monthsIncLength].substr(6, month.length);
        let incremetObj = {};
    ...
    let monthsIncLang = text.toString().match(/\:MMMM_[-,+]?[0-9]{1,2}_[a-zA-Z]{2}/ig);

    if (monthsIncLang) {
      let monthsLength = monthsIncLang.length;

      while (monthsLength--) {
        let month = monthsIncLang[monthsLength];
        let incLang = month.substr(6, month.length);
        let posInitLang = incLang.indexOf("_");
        let monthIncremet = incLang.substr(0,posInitLang);
        let monthLang = incLang.substr(posInitLang+1);
    ...
    let shortMonths = text.toString().match(/\:MMM_[a-zA-Z]{2}/ig);

    if (shortMonths) {
      let shortMonthsLength = shortMonths.length;

      while (shortMonthsLength--) {
        let shortMonth = shortMonths[shortMonthsLength].substr(1, 6);
        let shortMonthLang = shortMonths[shortMonthsLength].substr(5, 2);
    ...
    var shortMonthsInc = text.toString().match(/\:MMM_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (shortMonthsInc) {
      var shortMonthsIncLength = shortMonthsInc.length;

      while (shortMonthsIncLength--) {
        let shortMonth = shortMonthsInc[shortMonthsIncLength];
        let shortMonthInc = shortMonthsInc[shortMonthsIncLength].substr(5, shortMonth.length);
        let incremetObj = {};
        incremetObj.period = "months";
    ...
    let shortmonthsIncLang = text.toString().match(/\:MMM_[-,+]?[0-9]{1,2}_[a-zA-Z]{2}/ig);

    if (shortmonthsIncLang) {
      let shortMonthsLength = shortmonthsIncLang.length;

      while (shortMonthsLength--) {
        let shortMonth = shortmonthsIncLang[shortMonthsLength];
        let incLang = shortMonth.substr(5, shortMonth.length);
        let posInitLang = incLang.indexOf("_");
        let shortMonthIncremet = incLang.substr(0,posInitLang);
        let shortMonthLang = incLang.substr(posInitLang+1);
    ...
    // WEEKS WW_[INCREMENT]
    let weeksInc = text.toString().match(/\:WW_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (weeksInc) {
      let weeksIncLength = weeksInc.length;

      while (weeksIncLength--) {
        let week = weeksInc[weeksIncLength].substr(1, 5);
        let weekInc = weeksInc[weeksIncLength].substr(4, week.length);
        let incremetObj = {};
        incremetObj.period = "weeks";
        incremetObj.number = weekInc;
        if (!objParams[week]) {
          objParams[week] = getDateString("WW", true, "", incremetObj);
        }
      }
    }

    // DAYS DDDD_[LANG]
    let days = text.toString().match(/\:DDDD_[a-zA-Z]{2}/ig);
    ...
    let daysInc = text.toString().match(/\:DDDD_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (daysInc) {
      let daysIncLength = daysInc.length;

      while (daysIncLength--) {
        let day = daysInc[daysIncLength].substr(1, 7);
        let dayInc = daysInc[daysIncLength].substr(6, day.length);
        let incremetObj = {};
    ...
    let daysIncLang = text.toString().match(/\:DDDD_[-,+]?[0-9]{1,2}_[a-zA-Z]{2}/ig);

    if (daysIncLang) {
      let daysLength = daysIncLang.length;

      while (daysLength--) {
        let day = daysIncLang[daysLength];
        let incLang = day.substr(6, day.length);
        let posInitLang = incLang.indexOf("_");
        let dayIncremet = incLang.substr(0,posInitLang);
        let dayLang = incLang.substr(posInitLang+1);
    ...
    let shortDays = text.toString().match(/\:DDD_[a-zA-Z]{2}/ig);
    ...
    // DAYS DDD_[INCREMENT]
    let shortDaysInc = text.toString().match(/\:DDD_[-,+]?[0-9]{1,2}(?!_)\b/ig);

    if (shortDaysInc) {
      let shortDaysIncLength = shortDaysInc.length;

      while (shortDaysIncLength--) {
        let shortDay = shortDaysInc[shortDaysIncLength].substr(1, 6);
        let shortDayInc = shortDaysInc[shortDaysIncLength].substr(5, shortDay.length);
    ...
    let shortdaysInc = text.toString().match(/\:DDD_[-,+]?[0-9]{1,2}_[a-zA-Z]{2}/ig);

    if (shortdaysInc) {
      let shortdaysLength = shortdaysInc.length;

      while (shortdaysLength--) {
        let shortday = shortdaysInc[shortdaysLength];
        let incLang = shortday.substr(5, shortday.length);
        let posInitLang = incLang.indexOf("_");
        let shortdayIncremet = incLang.substr(0,posInitLang);
        let shortdayLang = incLang.substr(posInitLang+1);
...
function pick(object, allowed){
  ...
  var objPrepick = lodash.pick(object, allowed);
  var keys = Object.keys(objPrepick);
  for (const key of keys) {
    let prefKey = key + ".";
