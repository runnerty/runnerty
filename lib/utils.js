...
// async function interpretAwait(objParams, inputObject, params, options) {
//   try {
//     const interpret_res = await interpret(inputObject, params, options);
//     return interpret_res;
//   } catch (err) {
//     const msg = objParams.CHAIN_ID
//       ? 'CHAIN: ' + objParams.CHAIN_ID
//       : '' + objParams.PROCESS_ID
//       ? ' PROCESS: ' + objParams.PROCESS_ID
//       : '';
    ...
    if (ajv.getSchema('exec_' + executorId)) {
      ...
      try {
        ...
        if (valid) {
          ...
          reject(`Wrong parameters for the executor ${executor.id} (${executorId}): ${ajv.errorsText()}`);
        }
      } catch (err) {
    ...
    if (ajv.getSchema('trigger_' + triggerId)) {
      ...
      try {
        ...
        if (valid) {
          ...
          logger.log('error', `Checking trigger ${triggerId} \n${JSON.stringify(ajv.errors)}`);
