...
function replaceWithSmart(inputObject, objParams={}, options) {
  ...
  return new Promise(async (resolve) => {
    ...
    if (global.config && global.config.global_values) {
    ...
    chainsIds.forEach(async (chainId) => {
      ...
      if (_res) {
        ...
        if(program.custom_values){
          try {
            chain.custom_values = await objToKeyValue(JSON.parse(program.custom_values));
          } catch (err) {
        ...
        if(program.input_values){
          try {
            Object.assign(chain.input[0] || {},await objToKeyValue(JSON.parse(program.input_values)));
          } catch (err) {
