  ...
  return new Promise(async (resolve) => {
    ...
    if (monthsNumInc) {
      ...
      while (monthsNumIncLength--) {
        const month = monthsNumInc[monthsNumIncLength].substr(1, 5);
        const monthInc = monthsNumInc[monthsNumIncLength].substr(4, month.length);
        let incremetObj = {};
        incremetObj.period = "months";
        incremetObj.number = monthInc;
        if (!objParams[month]) {
          objParams[month] = getDateString("MM", true, "", incremetObj);
        }
      }
    }

    // MONTHS NUM. OUTPUT YEAR: MM_[INCREMENT]_YYYY
    const monthsNumIncY = text.toString().match(/\:MM_[-,+]?[0-9]{1,2}_YYYY/ig);
    ...
    if (monthsNumIncY) {
      ...
      while (monthsNumIncLength--) {
        ...
        const monthInc = monthsNumIncY[monthsNumIncLength].substr(4, month.length-9);
        console.log("- monthInc:",monthInc);
        let incremetObj = {};
        incremetObj.period = "months";
        incremetObj.number = monthInc;
        if (!objParams[month]) {
          objParams[month.substr(1)] = getDateString("YYYY", true, "", incremetObj);
          console.log("->",month,objParams[month.substr(1)]);
        }
      }
    }
    ...
    if (weeksInc) {
      ...
      while (weeksIncLength--) {
        const week = weeksInc[weeksIncLength].substr(1, 5);
