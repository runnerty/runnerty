...
function recursiveObjectInterpreter(inputObject, objParams = {}, options) {
  return new Promise(async resolve => {
    ...
    if (
      runtime.config &&
      runtime.config.general.interpreter_max_size &&
      sizeof(inputObject) > runtime.config.general.interpreter_max_size
    ) {
      logger.log('warn', ``);
      resolve(inputObject);
    }
    ...
    if (objParams && Object.keys(objParams).length !== 0) {
      if (!objParams.objParamsIsReplaced) {
        objParams.objParamsReplaced = await recursiveObjectInterpreter(objParams, {}, options);
    ...
    if (typeof inputObject === 'string') {
      ...
      if (inputObject instanceof Array) {
        ...
        for (let i = 0; i < inputObject.length; i++) {
          promArr.push(recursiveObjectInterpreter(inputObject[i], objParams, options));
        }
        ...
        if (inputObject instanceof Object) {
          ...
          for (const key of keys) {
            const _value = await recursiveObjectInterpreter(inputObject[key], objParams, options);
...
async function generateCalendar(filePath) {
  const fileName = path.parse(filePath).name;
  const fileExt = path.parse(filePath).ext;
  if (fs.existsSync(filePath)) {
    if (fileExt === '.ics') {
      let parsedCal = await ical.async.parseFile(filePath);
      const calEvents = [];
      for (const event in parsedCal) {
        calEvents.push(parsedCal[event]);
      }
...
module.exports.loadCalendars = async function loadCalendars(configFilePath) {
  runtime.calendars = {};
  let configCalendarsPath = runtime.config.general.calendarsPath;
  if (configCalendarsPath) {
    try {
      if (!fs.existsSync(configCalendarsPath)) {
        const configCalendarsPathAlt = path.join(path.dirname(configFilePath), configCalendarsPath);
        if (!fs.existsSync(configCalendarsPathAlt)) {
          throw new Error(`Calendars path not found in: ${configCalendarsPath} and ${configCalendarsPathAlt}`);
        } else {
          configCalendarsPath = configCalendarsPathAlt;
        }
      }

      const files = await fsp.readdir(configCalendarsPath);
      for (let i = 0; i < files.length; i++) {
        generateCalendar(path.join(configCalendarsPath, files[i]));
      }
    } catch (err) {
      throw err;
    }
...
function setUpServer(server) {
  ...
  return new Promise(resolve => {
    ...
    if (server.limit_req) {
      app.use(bodyParser.json({ limit: server.limit_req }));
    }
    // ================================================
  ...
  if (program.force_chain_exec) {
    ...
    for (const chainId of chainsIds) {
      ...
      if (_res) {
        ...
        logger.log(
          'error',
          `The chain you are trying to run (${chainId}) has not been found in the plan. Please check the chain ID.`
        );
      }
    }
  }
...
async function applyOuputFilter(filter, output) {
  if (filter instanceof Object) {
    ...
    if (filter[operator] instanceof Array) {
      switch (operator) {
        case '$or':
          ...
          for (const conditionFilter of filter[operator]) {
            const resFilter = await applyOuputFilter(conditionFilter, output);
        ...
        case '$and':
          res = output;
          for (const conditionFilter of filter[operator]) {
            res = await applyOuputFilter(conditionFilter, res);
          }
          break;
        default:
          break;
      ...
      res = await applyConditionOuputFilter(filter, output);
    }
    return res;
  } else {
...
async function applyConditionOuputFilter(condition, output) {
  const item = Object.keys(condition)[0];
  const operator = Object.keys(condition[item])[0];
  const value = await recursiveObjectInterpreter(condition[item][operator]);
  switch (operator) {
    case '$eq':
      return output.filter(i => i[item] == value);
    case '$match':
      return output.filter(i => typeof i[item] === 'string' && i[item].match(regExpFromString(value)));
    case '$lt':
      return output.filter(i => i[item] < value);
    case '$lte':
      return output.filter(i => i[item] <= value);
...
module.exports.applyOuputFilter = applyOuputFilter;

function applyOutputOrder(orderBy, output) {
  if (orderBy.length) {
    const orderFields = [];
...
module.exports.applyOutputOrder = applyOutputOrder;
