...
const csurf = require('csurf'); // Added csurf for CSRF protection
const app = express();
...
function requireDir(directory, modules) {
  return new Promise((resolve, reject) => {
    ...
    const sanitizedDirectory = path.normalize(containerDirectory).replace(/^(\.\.(\/|\\|$))+/, '');

    fs.readdir(sanitizedDirectory, (err, items) => {
      if (err) {
        reject(err);
      } else {
        if (items) {
          ...
          for (let i = 0; i < items.length; i++) {
            if (items[i].startsWith('@')) {
              const subDirs = fs.readdirSync(path.join(sanitizedDirectory, items[i]));
          ...
          while (dirsLength--) {
            if (fs.existsSync(path.join(sanitizedDirectory, dirs[dirsLength]))) {
              if (fs.statSync(path.join(sanitizedDirectory, dirs[dirsLength])).isDirectory()) {
                if (fs.existsSync(path.join(sanitizedDirectory, dirs[dirsLength], dirs[dirsLength] + '.js'))) {
                  const modulePath = path.join(sanitizedDirectory, dirs[dirsLength], dirs[dirsLength] + '.js');
                  container[dirs[dirsLength]] = require(modulePath);
                } else {
                  if (path.join(sanitizedDirectory, dirs[dirsLength], 'index.js')) {
                    container[dirs[dirsLength]] = require(path.join(sanitizedDirectory, dirs[dirsLength], 'index.js'));
                  }
                }
              }
            } else {
  ...
  return new Promise(resolve => {
    ...
    switch (true) {
      ...
      case !!server.unix_socket:
        srv = https.createServer(app); // Changed from http to https
    ...
    // = SECURITY =====================================
    app.use(helmet());
    app.use(csurf()); // Added csurf middleware for CSRF protection
