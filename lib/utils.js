...
function interpretAwait(objParams, inputObject, params, options) {
  return new Promise((resolve) => {
    interpret(inputObject, params, options)
      ...
      .catch((err) => {
        logger.log("error", "Interpreter:", objParams.CHAIN_ID ? "CHAIN: " + objParams.CHAIN_ID : "" + objParams.PROCESS_ID ? " PROCESS: " + objParams.PROCESS_ID : "", ": ", err, "IN:", inputObject);
        resolve();
      });
  });
}
...
function loadConfigModules(mergedConfig, configFilePath) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all([promiseNotifiersSchemas, promiseExecutorsSchemas, promiseTriggersSchemas])
      .then(() => {
          ...
          fs.readFile(filePath, "utf8", async (err, res) => {
            if (err) {
              ...
              validateConfig(configLoad)
                .then(() => {
                  resolve(replaceWithSmart(configLoad.config));
                })
                .catch(err => {
                  throw new Error(`Loading general validateConfig: ${JSON.stringify(err)}`);
                });
            }
...
function replaceWithSmart(inputObject, objParams = {}, options) {
  ...
  return new Promise(async (resolve) => {
    ...
    if (global.config && global.config.global_values && (!options || !options.ignoreGlobalValues)) {
...
function pick(object, allowed) {
  ...
  for (const key of keys) {
    let prefKey = key + ".";
    let subAllowed = allowed.map(function (item) {
          ...
          if (intialValue.format === "text") {
            ...
            if (intialValue.format === "json") {
              ...
              if (intialValue.value instanceof Object || intialValue.value instanceof Array) {
                res[keymaster + "_" + valueKey] = await interpretAwait(objParams, JSON.stringify(intialValue.value), objParams, rw_options);
...
module.exports.loadMongoHistory = function loadMongoHistory() {
  ...
  return new Promise((resolve) => {
    if (config.general.history && config.general.history.mongodb && (config.general.history.disable !== true)) {
      ...
      mongoose.connect(`mongodb://${config.general.history.mongodb.host}:${config.general.history.mongodb.port||"27017"}/${config.general.history.mongodb.database}`, {
          useMongoClient: true
        })
        .then(() => {
    ...
    if (ajv.getSchema("trigger_" + triggerId)) {
      ...
      try {
        ...
        if (valid) {
          ...
          logger.log("error", `Checking trigger ${triggerId} params: ${JSON.stringify(ajv.errors)}`);
          reject(ajv.errors);
        }
      } catch (err) {
...
function loadServers() {
  return new Promise((resolve, reject) => {
    if (global.config.general.servers && global.config.general.servers.length > 0) {
...
function JSON2KV(objectToPlain, separator, prefix) {
  ...
  for (let i = 0; i < eobjs.length; i++) {
    ...
    let key = prefix ? prefix + separator + eobjs[i].toUpperCase() : eobjs[i].toUpperCase();
    // Comprobamos si es un objeto:
    if (objectToPlain[eobjs[i]] && typeof objectToPlain[eobjs[i]] === 'object' && objectToPlain[eobjs[i]].constructor === Object) {
      ...
      if (Array.isArray(objectToPlain[eobjs[i]])) {
        ...
        let arrLength = objectToPlain[eobjs[i]].length;
        for (let z = 0; z < arrLength; z++) {
          // En caso de que el array tenga objetos:
          if (objectToPlain[eobjs[i]][z] && typeof objectToPlain[eobjs[i]][z] === 'object' && objectToPlain[eobjs[i]][z].constructor === Object) {
            // Llamada a la sub-función, incluyendo la posición del array en la key:
            _iterateObject(key + separator + z, objectToPlain[eobjs[i]][z]);

          } else {
            // Si no es un objeto incluimos en res el valor en la key con la posición del array:
            res[key + separator + z] = objectToPlain[eobjs[i]][z];
          }
        }
      } else {
